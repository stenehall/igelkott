 ...
 */
 PluginHandler.prototype.tryToLoad = function tryToLoad (file, config) {
  ...
  if (this.possiblePluginPaths.some(function(checkLoadPath) {
    return this._tryToLoad(file, config, checkLoadPath);
  }, this)) {
  }
  else {
...
PluginHandler.prototype._tryToLoad = function _load(file, config, pluginPath)
{
  var p = path.resolve(pluginPath, file+'.js');
  if (fs.existsSync(p))
  {
    this.load(file, config, require(p).Plugin);
    return true;
  }
  return false;
};